...
# BASE_PATH = "test_bbox"
BASE_PATH = "hand_detection/output/mediapipe/conf_0.40/camera05"
tmp = BASE_PATH
# BASE_PATH = "test_val/conf_0.7/camera04"
for i in range(5, 6):
    # img_idx = f"{i:06d}"
    for idx in range(320, 520):
        img_idx = f"{idx:06d}"
        camera = f"camera0{i}"
        ...
        if not os.path.exists(f"{BASE_PATH}/json/{img_idx}_test.json"):
            ...
            orig = cv2.imread(f"data/input/orbbec/{camera}/{idx:06d}.jpg")
            cv2.imwrite(f"test_comp/frame_{idx:06d}_{camera}.jpg", orig)
            continue
