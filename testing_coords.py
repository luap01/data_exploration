...
def main():
    path_to_trial = Path("data/")

    cameras = ["camera01", "camera02", "camera03", "camera04", "camera05", "camera06"]
    images = []

    # Load camera information
    cam_infos = load_cam_infos(path_to_trial, both=True)

    path = Path(f"{path_to_trial}/input/openpose/images/")
    ...
    for cam in cameras:
        ...
        if cam_id >= 5:
            img_path = str(path / f"camera{cam_id:02d}" / f"000000.jpg")
            ...
            if img is None:
                ...
                continue
            # Create a new image for undistorted result
